...
void Plant::DoSquashDamage()
{
    ...
    while (mBoard->IterateZombies(aZombie))
    {
        if ((aZombie->mRow == mRow || aZombie->mZombieType == ZombieType::ZOMBIE_BOSS) && aZombie->EffectedByDamage(aDamageRangeFlags))
        {
            ...
            {
                int aHealthBefore = aZombie->mBodyHealth + aZombie->mHelmHealth + aZombie->mShieldHealth;
                aZombie->TakeDamage(1800, 18U);
                int aHealthAfter = aZombie->mBodyHealth + aZombie->mHelmHealth + aZombie->mShieldHealth;
                mRLSquashDamage += std::max(0, aHealthBefore - aHealthAfter);
                mRLSquashHits++;
            }
        }
    }
...
Zombie* Plant::FindSquashTargetAt(Board* theBoard, LawnApp* theApp, int theRow, int theX, int theWidth, ZombieID theTargetZombieID)
{
    int aClosestRange = 0;
    Zombie* aClosestZombie = nullptr;
    Rect aAttackRect(theX + 20, theBoard->GridToPixelY(6, theRow), theWidth - 35, 80);

    Zombie* aZombie = nullptr;
    while (theBoard->IterateZombies(aZombie))
    {
        if ((aZombie->mRow == theRow || aZombie->mZombieType == ZombieType::ZOMBIE_BOSS) &&
            aZombie->mHasHead && !aZombie->IsTangleKelpTarget() && aZombie->EffectedByDamage(13))
        {
            Rect aZombieRect = aZombie->GetZombieRect();

            if ((
                    aZombie->mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT && aZombieRect.mX < theX + 20
                ) || (
                ...
                ))
            {
                ...
                if (aRange <= (aZombie->mIsEating ? 110 : 70))
                {
                    ...
                    if (aZombie->mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_POST_VAULT || aZombie->mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT ||
                        ...
                        aZombie->mZombieType == ZombieType::ZOMBIE_FOOTBALL || theApp->IsScaryPotterLevel())
                    {
                    ...
                    {
                        if (theBoard->ZombieGetID(aZombie) == theTargetZombieID)
                            return aZombie;
...
{
    return FindSquashTargetAt(mBoard, mApp, mRow, mX, mWidth, mTargetZombieID);
}
