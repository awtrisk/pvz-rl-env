...
namespace
{
int gCosmeticRandStateDepth = 0;

class CosmeticRandStateGuard
{
private:
	bool mOutermost;
	decltype(Sexy::GetRandState()) mState;

public:
	CosmeticRandStateGuard() : mOutermost(gCosmeticRandStateDepth++ == 0)
	{
		if (mOutermost)
			mState = Sexy::GetRandState();
	}

	~CosmeticRandStateGuard()
	{
		TOD_ASSERT(gCosmeticRandStateDepth > 0);
		gCosmeticRandStateDepth--;
		if (mOutermost)
			Sexy::SetRandState(mState);
	}
};
}
#endif
...
{
#ifdef HEADLESS
	CosmeticRandStateGuard aRandStateGuard;
#endif
...
void TodParticleEmitter::UpdateParticleField(TodParticle* theParticle, ParticleField* theParticleField, float theParticleTimeValue, int theFieldIndex)
{
	...
	case ParticleFieldType::FIELD_SHAKE:  // 震动
	{
		...
		// Particle shake must not reseed the process-wide C RNG: doing so makes
		// gameplay randomness depend on allocator addresses and render effects.
		unsigned int aLastShakeSeed = static_cast<unsigned int>(aLastRandSeed * 2654435761U);
		aLastShakeSeed ^= static_cast<unsigned int>(theParticle->mParticleDuration);
		float aLastShakeX = static_cast<float>((aLastShakeSeed * 1664525U + 1013904223U) & 0xFFFFU) / 32767.5f - 1.0f;
		float aLastShakeY = static_cast<float>((aLastShakeSeed * 22695477U + 1U) & 0xFFFFU) / 32767.5f - 1.0f;
		theParticle->mPosition.x -= aLastX * aLastShakeX;
		theParticle->mPosition.y -= aLastY * aLastShakeY;
		unsigned int aRandSeed = static_cast<unsigned int>(theParticle->mParticleAge * 2654435761U);
		aRandSeed ^= static_cast<unsigned int>(theParticle->mParticleDuration);
		float aShakeX = static_cast<float>((aRandSeed * 1664525U + 1013904223U) & 0xFFFFU) / 32767.5f - 1.0f;
		float aShakeY = static_cast<float>((aRandSeed * 22695477U + 1U) & 0xFFFFU) / 32767.5f - 1.0f;
		theParticle->mPosition.x += x * aShakeX;
		theParticle->mPosition.y += y * aShakeY;
		break;
	}
