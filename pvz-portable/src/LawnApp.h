...
public:
	...
	// The headless RL bridge never runs SexyAppBase::Process, so nothing
	// drains the widget safe-delete list between resets. Public wrapper so
	// the bridge can free the old Board right after KillBoard().
	void							DrainSafeDeleteList() { ProcessSafeDeleteList(); }
